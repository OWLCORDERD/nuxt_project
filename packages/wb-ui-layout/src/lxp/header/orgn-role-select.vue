<template>
  ...
  >
    ...
    <template #text>
      {{ selectedOrgn?.ORGN_NAME }} 학습자 홈으로 이동
    </template>
  </LXPButton>
  <div class="select-menu">
    <select
      class="orgn-select"
      aria-label="selectedOrgn"
      id="upperOrgnSelectBox"
      @change="changeUpperOrgn"
      :disabled="!['1', '2'].includes(roleId)"
    >
      <option
        v-for="orgn in orgnList.upper"
        :value="orgn.ORGN_ID"
        :selected="orgn.SELECTED === 'Y'"
      >
        {{ orgn.ORGN_NAME }}
      </option>
    </select>
    <select
      ...
      id="orgnSelectBox"
      @change="changeOrgn"
      :disabled="!['1', '2', '7'].includes(roleId)"
    >
      <option
        v-for="orgn in orgnList.lower"
        :value="orgn.ORGN_ID"
        :selected="orgn.SELECTED === 'Y'"
        :disabled="orgn.ORGN_ACCESS_AGREE_YN === 'N'"
      >
        {{ orgn.ORGN_NAME }}
      </option>
    </select>
    ...
    >
      <option
        ...
        :value="role.ROLE_ID"
        :selected="role.ROLE_ID === Number(roleId)"
      >
        {{ role.ROLE_NAME }}
      </option>
    </select>
